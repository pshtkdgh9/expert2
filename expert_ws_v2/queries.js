...
function uploadIFFile(req, res) {
	...
	if (type == 'kci') {
		...
		}).filter(x => x.IF != 0 && x.IF != null && x.IF != 'Not Available');
	}
        console.log(rows)
...
async function getIntRawResults(_data) {
	...
	return new Promise(async (resolve, reject) => {
		let _sites = ['NTIS', 'SCIENCEON', 'DBPIA', 'KCI'];
...
async function getIntegrationResult(req, res) {
	...
	if (fId == -1) {
		...
		dbs['ID'].collection('Domestic').findOne({
			"keyId": keyId,
			"fid": fId
		}, {}, function(err, result) {
			if (err) handleError(err, res);
